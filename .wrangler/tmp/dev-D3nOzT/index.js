...
// .wrangler/tmp/bundle-6ZJsi1/checked-fetch.js
...
// .wrangler/tmp/bundle-6ZJsi1/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-6ZJsi1/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// .wrangler/tmp/bundle-6ZJsi1/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
...
__name(handleHealthRequest, "handleHealthRequest");

// src/handlers/manifestHandler.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
...
var serverManifest = {
  ...
  api: {
    type: "jsonrpc",
    url: "https://better-beans-mcp.workers.dev/rpc"
...
// src/handlers/rpcHandler.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// src/auth/apiKeyValidation.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
...
async function validateApiKey(apiKey, env) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("api_keys").select("id, permissions, rate_limit, is_active, expires_at").eq("key_hash", keyHash).single();
    if (error) {
    ...
    await supabase.from("api_keys").update({ last_used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", data.id);
    return {
      valid: true,
...
async function hashApiKey(apiKey, salt) {
  const encoder = new TextEncoder();
...
// src/services/methodRouter.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// src/services/roasterService.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
...
// src/services/roasterService.ts
var RoasterSearchParamsSchema = external_exports.object({
  query: external_exports.string().min(1).max(200),
  maxResults: external_exports.number().int().positive().max(100).optional().default(10),
  offset: external_exports.number().int().min(0).optional().default(0)
});
function validateRoasterSearchParams(params) {
  return RoasterSearchParamsSchema.parse(params);
}
__name(validateRoasterSearchParams, "validateRoasterSearchParams");
var searchRoastersSchema = external_exports.object({
  query: external_exports.string().optional(),
  location: external_exports.string().optional(),
  maxResults: external_exports.number().int().min(1).max(50).default(10),
  filters: external_exports.object({
    organic: external_exports.boolean().optional(),
    fairTrade: external_exports.boolean().optional(),
    directTrade: external_exports.boolean().optional(),
    subscription: external_exports.boolean().optional()
  }).optional()
});
var roasterDetailsSchema = external_exports.object({
  roasterId: external_exports.string().uuid()
});
async function searchCoffeeRoasters(params, env) {
  ...
  try {
    ...
    let query = supabase.from("roasters").select(`
        id,
        roaster_name,
        city,
        state,
        ...
        logo_url,
        is_featured
      `).eq("is_active", true).order("is_featured", { ascending: false }).limit(validParams.maxResults);
...
__name(searchCoffeeRoasters, "searchCoffeeRoasters");
async function getRoasterDetails(params, env) {
  ...
  try {
    const supabase = getSupabaseClient(env);
    const { data: roaster, error: roasterError } = await supabase.from("roasters").select("*").eq("id", validParams.roasterId).eq("is_active", true).single();
    if (roasterError) {
    ...
    const { data: coffees, error: coffeesError } = await supabase.from("coffees").select(`
        id,
        coffee_name,
        roast_level,
        process_method,
        ...
        image_url,
        is_available
      `).eq("roaster_id", validParams.roasterId).eq("is_available", true).order("coffee_name");
    if (coffeesError) {
...
__name(getRoasterDetails, "getRoasterDetails");

// src/services/coffeeService.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var searchCoffeeProductsSchema = external_exports.object({
  query: external_exports.string().optional(),
  roasterId: external_exports.string().uuid().optional(),
  maxResults: external_exports.number().int().min(1).max(50).default(10),
  filters: external_exports.object({
    processMethod: external_exports.string().optional(),
    roastLevel: external_exports.string().optional(),
    origin: external_exports.array(external_exports.string()).optional(),
    flavorProfile: external_exports.array(external_exports.string()).optional(),
    isCaffeinated: external_exports.boolean().optional()
  }).optional()
});
var coffeeDetailsSchema = external_exports.object({
  productId: external_exports.string().uuid()
});
async function searchCoffeeProducts(params, env) {
  ...
  try {
    ...
    let query = supabase.from("coffees").select(`
      ...
      `).eq("is_available", true).limit(validParams.maxResults);
    if (validParams.query) {
    ...
    if (validParams.filters) {
      ...
      if (validParams.filters.origin && validParams.filters.origin.length > 0) {
        const originConditions = validParams.filters.origin.map(
      ...
      if (validParams.filters.flavorProfile && validParams.filters.flavorProfile.length > 0) {
        const flavorConditions = validParams.filters.flavorProfile.map(
          (flavor) => `flavor_tags.cs.{${flavor}}`
        ).join(",");
        query = query.or(flavorConditions);
      }
      if (validParams.filters.isCaffeinated !== void 0) {
    ...
    return {
      coffees: data.map((coffee) => ({
        ...
        roaster: {
          id: coffee.roasters?.id || null,
          name: coffee.roasters?.roaster_name || null
        }
      })),
      totalResults: data.length
    };
...
__name(searchCoffeeProducts, "searchCoffeeProducts");
async function getCoffeeProductDetails(params, env) {
  ...
  try {
    const supabase = getSupabaseClient(env);
    const { data: coffee, error } = await supabase.from("coffees").select(`
        *,
        roasters(id, roaster_name, city, state, logo_url)
      `).eq("id", validParams.productId).eq("is_available", true).single();
    if (error) {
    ...
    return {
      ...
      roaster: coffee.roasters ? {
        ...
        location: coffee.roasters.city && coffee.roasters.state ? `${coffee.roasters.city}, ${coffee.roasters.state}` : null,
...
__name(getCoffeeProductDetails, "getCoffeeProductDetails");

// src/services/semanticService.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var SimilaritySearchParamsSchema = external_exports.object({
  flavorProfile: external_exports.array(external_exports.string()).min(1).max(10),
  maxResults: external_exports.number().int().positive().max(100).optional().default(10),
  offset: external_exports.number().int().min(0).optional().default(0)
});
function validateSimilaritySearchParams(params) {
  return SimilaritySearchParamsSchema.parse(params);
}
__name(validateSimilaritySearchParams, "validateSimilaritySearchParams");
async function similaritySearch(params, env) {
  try {
    const validParams = validateSimilaritySearchParams(params);
    const supabase = getSupabaseClient(env);
    const { data: coffeeProducts, error: queryError } = await supabase.from("coffee_products").select(`
    ...
    const results = coffeeProducts.map((product) => {
      ...
      const matchingTags = validParams.flavorProfile.filter(
        (tag) => flavorTags.includes(tag)
      );
    ...
    }).filter((result) => result.similarityScore > 0).sort((a, b) => b.similarityScore - a.similarityScore);
    return {
      query: { flavorProfile: validParams.flavorProfile },
      results,
...
__name(handleStreamingSimilaritySearch, "handleStreamingSimilaritySearch");
...
var MethodNotFoundError = class extends Error {
  static {
    __name(this, "MethodNotFoundError");
  }
  constructor(message) {
    super(message);
...
var StreamingNotSupportedError = class extends Error {
  static {
    __name(this, "StreamingNotSupportedError");
  }
  constructor(message) {
    super(message);
...
async function methodRouter(method, params = {}, env, options) {
...
__name(methodRouter, "methodRouter");
...
__name(handleStreamingRoasterSearch, "handleStreamingRoasterSearch");
...
__name(handleStreamingProductSearch, "handleStreamingProductSearch");
...
async function writeJsonRpcSuccessToStream(writer, result, id, isFinal = false) {
  const response = {
    jsonrpc: "2.0",
    result,
    id
...
__name(writeJsonRpcSuccessToStream, "writeJsonRpcSuccessToStream");
async function writeJsonRpcErrorToStream(writer, error, id) {
  const response = {
    jsonrpc: "2.0",
    error,
    id
...
__name(writeJsonRpcErrorToStream, "writeJsonRpcErrorToStream");
async function closeSseStream(writer) {
  try {
    await writer.close();
  } catch (error) {
...
__name(closeSseStream, "closeSseStream");
...
async function handleRpcRequest(request, env, ctx) {
  ...
  try {
    if (isStreamingRequested) {
      ...
      if (requestParam) {
        try {
          rpcRequest = JSON.parse(requestParam);
        } catch (parseError) {
...
__name(createJsonRpcSuccessResponse, "createJsonRpcSuccessResponse");
function createJsonRpcErrorResponse(error) {
  ...
  return createCorsResponse(
    new Response(JSON.stringify(jsonRpcError), {
      status: 200,
      // JSON-RPC uses 200 OK even for errors
      headers: {
        "Content-Type": "application/json"
      }
    })
...
async function handleStreamingRequest(rpcRequest, env, ctx) {
  const [response, writer] = createSseStream({
    onClose: /* @__PURE__ */ __name(() => console.log(`Stream closed for request ${rpcRequest.id}`), "onClose")
  });
  ctx.waitUntil(
    (async () => {
      try {
        const streamResult = await methodRouter(
          ...
          {
            streaming: true,
            onPartialResult: /* @__PURE__ */ __name(async (partialResult) => {
              try {
...
function isValidJsonRpcRequest(request) {
  return request && request.jsonrpc === "2.0" && typeof request.method === "string" && (request.id === void 0 || typeof request.id === "string" || typeof request.id === "number") && (request.params === void 0 || typeof request.params === "object");
}
__name(isValidJsonRpcRequest, "isValidJsonRpcRequest");

// src/handlers/sseHandler.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var STREAMING_SUPPORTED_METHODS = [
...
async function handleSseRequest(request, env, ctx) {
    ...
    return createCorsResponse(
      new Response(
        JSON.stringify({
          jsonrpc: "2.0",
          error: {
            code: ERROR_CODES2.INVALID_REQUEST,
  ...
  if (!isValidJsonRpcRequest2(rpcRequest)) {
    return createCorsResponse(
      new Response(
        JSON.stringify({
          jsonrpc: "2.0",
      ...
      return createCorsResponse(
        new Response(
          JSON.stringify({
            jsonrpc: "2.0",
            error: {
              code: ERROR_CODES2.UNAUTHORIZED,
              message: error || "Unauthorized: Invalid API key"
            },
            id: rpcRequest.id
          }),
    ...
    (async () => {
      try {
        ...
        if (error instanceof Error) {
          ...
          } else if (error.code === "STREAMING_NOT_SUPPORTED" || !STREAMING_SUPPORTED_METHODS.includes(rpcRequest.method)) {
            errorCode = ERROR_CODES2.STREAMING_NOT_SUPPORTED;
            errorMessage = `Streaming is not supported for method '${rpcRequest.method}'`;
            errorData = {
        ...
        await writer.write(new TextEncoder().encode(
          formatSseMessage(
            ...
            "error"
            // Use a specific event type for errors
          )
        ));
        await writer.write(new TextEncoder().encode(
          formatSseMessage(
...
function isValidJsonRpcRequest2(request) {
  return request && request.jsonrpc === "2.0" && typeof request.method === "string" && (request.params === void 0 || typeof request.params === "object") && (typeof request.id === "string" || typeof request.id === "number");
}
__name(isValidJsonRpcRequest2, "isValidJsonRpcRequest");
